     ...
     (define names-because-implicit-assignments/or-false
       (for/or ([bound-expr (in-list es)])
         (match bound-expr
           ...
           [els
            (log-status "WARN: unable to show that implicit assignment in letrec is unobservable")
            (let ([debug #t])
              (when debug
                  (log-status "(names ~a) (bound-exprs ~a) (body ~a)" name* es e)))
